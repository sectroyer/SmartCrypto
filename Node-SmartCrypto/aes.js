 ...
 * @constructor
 */
function GetU32(pt, offset) {
    return (((pt)[offset] << 24) ^ ((pt)[1 + offset] << 16) ^ ((pt)[2 + offset] << 8) ^ ((pt)[3 + offset]));
}
 ...
 * @param {Uint8Array} cipherKey
 * @param {number} keyBits
 * @return {number} the number of rounds for the given cipher key size.
 */
function rijndaelKeySetupEncTransform(nr, rk, cipherKey, keyBits) {
    let i = 0;
    let temp;
 ...
 * @param {Uint32Array} rk
 * @param {number} nr
 * @param {Uint8Array} pt - Plain text
 * @param {Uint8Array} ct - Cipher text
 */
function rijndaelEncrypt(rk, nr, pt, ct) {
    // Offset into rk
    let off = 0;

    let s0 = GetU32(pt, 0) ^ rk[off];
    let s1 = GetU32(pt, 4) ^ rk[off + 1];
    let s2 = GetU32(pt, 8) ^ rk[off + 2];
    let s3 = GetU32(pt, 12) ^ rk[off + 3];

    // Round 1
    let t0 = Te0[s0 >>> 24] ^ Te1[(s1 >>> 16) & 0xff] ^ Te2[(s2 >>>  8) & 0xff] ^ Te3[s3 & 0xff] ^ rk[off + 4];
    let t1 = Te0[s1 >>> 24] ^ Te1[(s2 >>> 16) & 0xff] ^ Te2[(s3 >>>  8) & 0xff] ^ Te3[s0 & 0xff] ^ rk[off + 5];
    let t2 = Te0[s2 >>> 24] ^ Te1[(s3 >>> 16) & 0xff] ^ Te2[(s0 >>>  8) & 0xff] ^ Te3[s1 & 0xff] ^ rk[off + 6];
    let t3 = Te0[s3 >>> 24] ^ Te1[(s0 >>> 16) & 0xff] ^ Te2[(s1 >>>  8) & 0xff] ^ Te3[s2 & 0xff] ^ rk[off + 7];
    
    // Round 2
    s0 = Te0[t0 >>> 24] ^ Te1[(t1 >>> 16) & 0xff] ^ Te2[(t2 >>>  8) & 0xff] ^ Te3[t3 & 0xff] ^ rk[off +  8];
    s1 = Te0[t1 >>> 24] ^ Te1[(t2 >>> 16) & 0xff] ^ Te2[(t3 >>>  8) & 0xff] ^ Te3[t0 & 0xff] ^ rk[off +  9];
    s2 = Te0[t2 >>> 24] ^ Te1[(t3 >>> 16) & 0xff] ^ Te2[(t0 >>>  8) & 0xff] ^ Te3[t1 & 0xff] ^ rk[off + 10];
    s3 = Te0[t3 >>> 24] ^ Te1[(t0 >>> 16) & 0xff] ^ Te2[(t1 >>>  8) & 0xff] ^ Te3[t2 & 0xff] ^ rk[off + 11];
 ...
 * @return {Uint8Array}
 */
export function AES_128_Transform(nr, key, plainText, cipherText) {

    const aes128rounds = 10;

    const len = 4 * (aes128rounds + 1);
    const rk = new Uint32Array(len);
